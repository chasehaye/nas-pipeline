...
func (s *Store) UpsertFlight(ctx context.Context, f flight.Flight) error {
	...
	if f.Position.HasHeading {
		fields["heading"] = f.Position.Heading
		fields["speedKt"] = f.Position.SpeedKt
	}
...
func (s *Store) DeleteFlight(ctx context.Context, gufi string) error {
	return s.rdb.Del(ctx, s.prefix+gufi).Err()
}
